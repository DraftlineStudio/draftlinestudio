...
export default function StorySearchToolWindow() {
  ...
  function navigate(match: SearchMatch) {
    setViewMode('editor')
    setCurrentChapter(match.section as Section, match.section_index)
    window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent('draftline:find-story-evidence', { detail: { query: match.matched_terms[0] ?? '' } }))
    }, 0)
  }
  ...
  return (
    <section className="story-search-window" style={{ height: panelHeight }} aria-label="Story Search">
      ...
      <header className="story-search-header">
        <div className="story-search-tab">
        ...
        <span className="story-search-header-hint">Local evidence search · no AI</span>
        <button className="story-search-close" onClick={close} title="Close Story Search" aria-label="Close Story Search">×</button>
      </header>

      <form className="story-search-form" onSubmit={submit}>
        <div className="story-search-input-wrap">
          ...
          <input
            ...
            placeholder={'Try Hanlon Ruiz IBM, or "underground tunnel"'}
      ...
      <div className="story-search-body">
        {!result && !loading && (
