...
export function exportTextNote(
  ...
  if (!snapshot) {
    return 'No text is frozen for this ISBN yet, so this export uses your manuscript as it stands today — and freezes it, so that exporting this edition again after you have carried on writing gives you these same words back.'
  }
  const when = frozenLabel(snapshot.frozen)
  return `Exports the text frozen for this ISBN${when ? ` on ${when}` : ''} — ${snapshot.word_count.toLocaleString()} words — and not the draft you have open. Everything else, from the cover to the copyright fields, comes from the edition record as it stands now.`
}
