...
func frameBook(records []types.EvidenceRecord) types.BookData {
	...
	book.Analysis.EntityResolution = &types.EntityData{Entities: []types.EntityRecord{
		...
		{ID: "entity-2", Canonical: "Mira Brannick", Aliases: []string{"Mira Brannick", "Mira", "Brannick"}},
...
func TestEveryDetailIsVerbatimAndEveryFrameHasEvidence(t *testing.T) {
	book := frameBook([]types.EvidenceRecord{
		...
		evidence("e2", 0, "Mira Brannick was killed in the warehouse fire.", "event", "state", "killed"),
...
func TestLifeStatusLocationPossessionInjury(t *testing.T) {
	book := frameBook([]types.EvidenceRecord{
		evidence("e1", 0, "Mira Brannick was killed in the warehouse fire.", "event", "state", "killed"),
	...
	if life == nil || life.Value != "dead" || subjectOf(life) != "Mira Brannick" {
	...
	if injury == nil || subjectOf(injury) != "Mira Brannick" {
...
func TestAttributedClaimNeverBecomesNarratorTruth(t *testing.T) {
	...
	if claim.Epistemic != "attributed_claim" || claim.Attribution.Kind != "character" || claim.Attribution.EntityName != "Mira Brannick" {
...
func TestKnowledgeStatesProjectWithCanonicalKnower(t *testing.T) {
	...
	record.KnowledgeStates = []types.EvidenceKnowledgeState{{State: "suspects", CharacterNames: []string{"Mira Brannick"}, Cue: "suspected the audit was staged", Confidence: .8}}
	book := frameBook([]types.EvidenceRecord{record})
	frames := extract(t, book)
	belief := frameOfType(frames, types.FrameBelief)
	if belief == nil || subjectOf(belief) != "Mira Brannick" || belief.Epistemic != "belief" {
