...
| File | Contents |
|------|----------|
| `book.go` | Core book structures: `BookData`, `Metadata`, `ChapterItem`, `WritingGoals`, `WritingStyleOptions`, `ReadAloudCast`, and the Planner records (`PlannerData`, `PlannerLane`, `PlannerCard`, `PlannerNote`, `PlannerLink`) |
...
| `entities.go` | Entity resolution records and the `AnalysisData` container (entity resolution, relationships, story metrics, evidence, continuity decisions) |
