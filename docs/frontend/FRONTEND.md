...
**Read Aloud** (`draftline.read-aloud`) is the first shipped `optional-model` plugin: fully local Kokoro TTS with per-sentence editor highlighting, a one-time verified ~130 MB download managed by Go, and a synthesis worker that only loads while the plugin is enabled. See `docs/frontend/READ-ALOUD.md`.
